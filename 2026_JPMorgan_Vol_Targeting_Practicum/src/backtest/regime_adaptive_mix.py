...
OBJECTIVE_ORDER = ["Sharpe", "MaxDrawdown", "CVaR", "Turnover"]
...
class RollingRegimeMixEngine(Engine):
    ...
    def _fit_hmm_regimes(
        ...
        transition_update_years = int(self.config.get("hmm_transition_update_years", 1))
        ...
        if as_of_date is not None and transition_update_years > 0:
            bucket_year = int(as_of_date.year // transition_update_years)
            update_key = (input_kind, bucket_year)
    ...
    def _build_winner_table(
        ...
        for regime in REGIME_ORDER:
            ...
            distinct_risk_pool = bool(
                self.config.get("distinct_risk_pool_from_sharpe", True)
            )
            min_sharpe_gap = float(self.config.get("risk_objective_min_sharpe_gap", 0.05))
            ...
            for objective in ["MaxDrawdown", "CVaR"]:
                ...
                filtered_metric_values = metric_values.copy()

                if distinct_risk_pool and sharpe_best_strategy is not None and not np.isnan(sharpe_best_value):
                    strict_mask = sharpe_values < float(sharpe_best_value - min_sharpe_gap)
                    strict_candidates = filtered_metric_values.loc[
                        strict_mask.reindex(filtered_metric_values.index).fillna(False)
                    ]

                    if not strict_candidates.empty:
                        filtered_metric_values = strict_candidates
                    else:
                        fallback_candidates = filtered_metric_values.drop(
                            labels=[sharpe_best_strategy], errors="ignore"
                        )
                        if not fallback_candidates.empty:
                            filtered_metric_values = fallback_candidates

                best_strategy, best_value = self._pick_best(
                    filtered_metric_values, objective=objective
                )
                rows.append(
                    {
                        "as_of_date": as_of_date,
            ...
            turnover_best_strategy, turnover_best_value = self._pick_best(
                turnover_values, objective="Turnover"
            )
            rows.append(
                {
                    "as_of_date": as_of_date,
