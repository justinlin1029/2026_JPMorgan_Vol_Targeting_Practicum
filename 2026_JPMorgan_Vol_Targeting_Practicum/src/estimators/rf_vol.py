...
class RandomForestVolEstimator(Estimator):
    ...
    def __init__(self, params=None):
        ...
        self.train_window  = self.params.get("train_window", 252)
        self.rv_window     = self.params.get("rv_window", 5)
        self.use_hmm       = self.params.get("use_hmm", True)
        self.n_iter        = self.params.get("n_iter", 3)
        self.tune_period   = self.params.get("tune_period", 21)
        self.hmm_model     = None
        self.hmm_update_days = 22
        self.auto_tune     = self.params.get("auto_tune", True)
        self.cv_splits     = self.params.get("cv_splits", 3)
    ...
    def estimate_window(self, window_returns: pd.Series) -> float:
        ...
        try:
            ...
            if self.auto_tune:
                if self._best_model is None or self._count % self.tune_period == 0:
