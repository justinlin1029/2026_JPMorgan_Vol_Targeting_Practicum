...
class XGB_VIX(Estimator):
    ...
    def _build_features(self, r: pd.Series, shift_for_training: bool = True) -> pd.DataFrame:
        ...
        # ── 1. Returns-based RV features (use returns_clean when available) ──
        # Prefer returns_clean to avoid roll-day spikes inflating variance proxies
        r_clean = self._get_returns_clean(dates, r)
    ...
    def estimate_window(self, window_returns: pd.Series) -> float:
        ...
        try:
            ...
            log_var_hat = float(model.predict(X_pred)[0])
